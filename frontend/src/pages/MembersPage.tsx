...
import { gymService, memberService, attendanceService, workoutService, authService } from '../services/api';
...
function blankForm() {
  return { name: '', email: '', phoneNumber: '', trainingDays: [] as string[], target: '', monthlyAmount: '', trainerUserIds: [] as number[] };
}
...
function MemberForm({
  ...
  return (
    <form className="space-y-4" onSubmit={onSubmit}>
      ...
      {allowTrainerAssignment && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Assign Trainer(s)</label>
          {trainers.length === 0 ? (
            <p className="text-xs text-slate-400">No trainers found yet. Create trainers first in Staff.</p>
          ) : (
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              {trainers.map((trainer) => {
                const checked = form.trainerUserIds.includes(trainer.id);
                return (
                  <label
                    key={trainer.id}
                    className={`flex items-center gap-2 rounded-md border px-3 py-2 text-sm transition ${
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const nextIds = e.target.checked
                          ? [...form.trainerUserIds, trainer.id]
                          : form.trainerUserIds.filter((id) => id !== trainer.id);
                        onChange({ ...form, trainerUserIds: nextIds });
                      }}
                    />
...
export default function MembersPage() {
  ...
  const loadMembers = useCallback(async () => {
    ...
    try {
      ...
      if (!isTrainer) {
        try {
          const trainersRes = await authService.listTrainers();
          setTrainers((trainersRes.data.data || []) as TrainerUser[]);
        } catch {
          setTrainers([]);
        }
      }
  ...
  const handleAddMember = async (e: React.FormEvent) => {
    ...
    try {
      const res = await memberService.create({
        ...
        trainer_user_ids: addForm.trainerUserIds,
      });
  ...
  const openEditModal = (member: Member) => {
    ...
    setEditForm({
      ...
      trainerUserIds: member.trainer_user_ids || [],
    });
